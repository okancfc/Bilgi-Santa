...
export default function MemoriesPage() {
  ...
  const handleUploadMemory = async () => {
    ...
    if (hasPairMemory) {
      setMemoryMessage({ type: "error", text: "Eşleşmeniz için zaten bir fotoğraf var. Önce mevcut fotoğrafı silin." })
      return
    }
  ...
  const handleDeleteMemory = async (memoryId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("Memory delete error:", text)
        setMemoryMessage({ type: "error", text: "Fotoğraf silinemedi." })
        return
      }
      setMemories((prev) => prev.filter((m) => m.id !== memoryId))
      setMemoryMessage({ type: "success", text: "Fotoğraf silindi. Yeniden yükleyebilirsin." })
    } catch (error) {
      console.error("Memory delete flow error:", error)
      setMemoryMessage({ type: "error", text: "Fotoğraf silinirken bir hata oluştu." })
    } finally {
          ...
          >
            {memories.map((memory) => {
              ...
              >
                ...
                {(memory.user_id === userId || (partnerId && memory.user_id === partnerId)) && (
                  <div className="absolute top-4 left-4 z-10">
                    <button
                      type="button"
                      onClick={() => handleDeleteMemory(memory.id)}
                      disabled={deletingMemoryId === memory.id}
                      className="w-11 h-11 rounded-full bg-black/60 text-white border border-border hover:bg-black/70 transition-colors flex items-center justify-center disabled:opacity-60"
